...
const source = fs.readFileSync(
    path.join(process.cwd(), "js/thunder.js"),
    "utf8"
)
...
jscs.registerMethods({
    findClass: function (name) {
        return this.find(jscs.ClassDeclaration).filter(
...
jscs.registerMethods(
    {
        findMethod: function (name) {
            return this.find(jscs.MethodDefinition).filter(
                (path) => path.value.key.name === name
            )
        },
    },
...
jscs.registerMethods(
    {
        ...
        findAssignment: function (name, this_bool = false) {
            return this.find(jscs.AssignmentExpression).filter((path) => {
                if (this_bool) {
                    return (
                        path.value.left.property.name === name &&
...
jscs.registerMethods(
    {
        classVariable: function (name) {
            return this.find(jscs.MemberExpression).filter(
                (path) =>
                    path.value.property.name === name &&
...
const source_html = fs.readFileSync(
...
const match = (obj, match_obj) =>
    jscs.match(obj.get().value, dot.object(match_obj))
